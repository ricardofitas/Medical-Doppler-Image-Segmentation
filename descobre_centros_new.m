...
function [centro,areas,evt,CCC,S]=descobre_centros_new(Luminance)
start=tic;
centro=[];
w=0;
[Matriz_x , Matriz_y, guardar_pontos,ev]=segmentar_3(Luminance,0,1,size(Luminance,2)*size(Luminance,1));
evt=ev;
...
while S>0
    w=w+1;
    [Matriz_x , Matriz_y, guardar_pontos,ev]=segmentar_3(Luminance,0,1,S(end));
    evt=[evt ev];
